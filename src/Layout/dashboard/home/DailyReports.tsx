...
export default function DailyReports() {
  ...
  schedule.forEach((item) => {
    ...
    completionTally[dateKey] = completionTally[dateKey] || { total: 0, completed: 0 };
  ...
  if (isMobile) {
    const selectedDateIndex = calendarDates.findIndex(
      ({ date }) => selectDate.toDate().toDateString() === date.toDate().toDateString()
    );
    if (selectedDateIndex !== -1) {
      reportInjectionIndex = Math.min((Math.floor(selectedDateIndex / 7) + 1) * 7, calendarDates.length);
    }
  }

  const calendarElements = calendarDates.reduce((acc, { date, currentMonth, today: isToday }, idx) => {
    // Mobile Injection Point
    if (isMobile && idx === reportInjectionIndex) {
      acc.push(
        <div key="mobile-reports-insert" className="col-span-7 mt-2 mb-2 bg-white rounded-lg p-2">
      ...
      >
        <div
          className={cn(
            ...
            currentMonth && !isSelected && !isTodayDate && "text-gray-700 hover:bg-gray-100",
            currentMonth && isTodayDate && "bg-blue-500 text-white ring-4 ring-blue-300 shadow-lg scale-110",
            currentMonth && isSelected && "bg-pink-500 text-white ring-4 ring-pink-300 shadow-xl scale-110"
          )}
        >
          {date.date()}
        </div>
  ...
  if (isMobile && reportInjectionIndex === calendarDates.length) {
    calendarElements.push(
      <div key="mobile-reports-insert-end" className="col-span-7 mt-2 mb-2 bg-white rounded-lg p-2">
        <Reports today={today} selectDate={selectDate} isMobile={true} />
      </div>
    );
  }
      ...
      <div className="flex flex-col w-full ip:max-w-[520px] bg-white rounded-3xl p-6 gap-5 border border-gray-200">
        ...
        <div className="flex justify-between items-center mb-4 gap-2">
          ...
          <div className="flex gap-2 items-center">
            <select
              value={today.month()}
              onChange={(e) => setToday(today.month(parseInt(e.target.value)))}
