  ...
  const timeInput = formData.time.map((item: string, index: number) => {
    return (
      <input
        ...
        disabled={loading}
        className="w-full px-4 py-2 border border-gray-300 dark:border-slate-800 rounded-lg text-sm bg-gray-50/50 dark:bg-slate-900/50 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500"
      />
    );
  });
      ...
      {activeModal === "edit" && (
        ...
        >
          ...
          <motion.div
            ...
            className="w-full max-w-lg bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl shadow-2xl max-h-[90vh] flex flex-col"
          >
            {/* Header */}
